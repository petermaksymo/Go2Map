...
std::vector<CourierSubpath> traveling_courier(
    ...
    #pragma omp parallel 
    {
        ...
        while(!timeOut) {
            ...
            if(is_legal && (new_time < min_time || (temp < TIME_LIMIT && (pcg32_fast() % temp == 0))) // for simulated annealing
            ) {
                min_time = new_time;
            } else {
            ...
            int x = wallClock.count()/10;
            
            temp = 1 + x + x*x/2 ;//+ x*x*x/6;
